...
#     #handle_msg.handle_all_message({},message)
#     # .onmessage({"method":"POST",
#     # "body":json.dumps(message)
#     # })
#     # url = host+"/bot/onmessage/"
#     # headers = {
#     # 'Content-Type': 'application/json'
#     # }
#     # print("监听消息事件，准备POST消息数据===>",json.dumps(message))
#     # requests.request("POST", url, headers=headers, data = json.dumps(message))
...
# #                                     r'^['+bot_name+']*[\s\?]*[舔,赞,顶,捧,夸,谢谢,感谢]+(.+)', msg_content).group(1).replace("@", "").replace("?", "")
...
# #                             elif re.match(r'^['+bot_name+']+[\s\?]+(我是色批|来张色图|我要看图片)', msg_content) and from_chatroom_wxid in group_receive_list_vip:
# #                                 wx_inst.send_img(from_chatroom_wxid,randomFile(isLoad=False))
# #                                 pass
# #                             elif re.match(r'^['+bot_name+']+[\s\?]*[更新图片]+', msg_content):
# #                                 if from_wxid != admin_wx:
# #                                     wx_inst.send_text(from_chatroom_wxid,"此命令您没权限！")
# #                                 oldLen=len(filePathList)
# #                                 randomFile(isLoad=True)
# #                                 wx_inst.send_text(
# #                                     from_chatroom_wxid, "原来共有"+str(oldLen)+"张图片，更新后有"+str(len(filePathList)))                                
...
# #                             elif re.match(r'[help,帮助]+.*['+bot_name+']+[\s\?]+[help,帮助]+', msg_content):
...
# #                                 to_my_msg = msg_content.replace(bot_name+"?",'').replace(bot_name+" ",'')
# #                                 print('to_my_msg',to_my_msg)
# #                                 response = requests.get(
...
# #                                 add_group_receive_config({from_chatroom_wxid:from_chatroom_nickname},"group_receive_list_dic")
# #                                 print('添加群组2',{from_chatroom_wxid:from_chatroom_nickname})
# #                                 wx_inst.send_text(from_chatroom_wxid, "群组【"+from_chatroom_nickname+"】添加成功！") 
# #                             if re.match(r'^[添加vip群组]+', msg_content) and from_wxid == admin_wx :
# #                                 msg_content = message.get('data', {}).get('msg', '')
# #                                 from_wxid = message.get('data', {}).get('from_member_wxid', '')
# #                                 from_chatroom_wxid = message.get('data', {}).get('from_chatroom_wxid', '')
# #                                 from_chatroom_nickname = message.get('data', {}).get('from_chatroom_nickname', '')
# #                                 from_chatroom_nickname = message.get('data', {}).get('from_chatroom_nickname', '')
# #                                 #global group_receive_list
# #                                 group_receive_list_vip.append(from_chatroom_wxid)
# #                                 add_group_receive_config({from_chatroom_wxid:from_chatroom_nickname},"group_receive_list_dic_vip")
# #                                 wx_inst.send_text(from_chatroom_wxid, "vip群组【"+from_chatroom_nickname+"】添加成功！") 
...
#     print(len(filePathList)-1)
#     print(filePathList[random.randint(0,len(filePathList)-1)])
...
# def add_group_receive_config(dic,key):
#     path = get_config_path()
#     try:
#         f = open(get_config_path(), "rb")
#         j = json.loads(f.read())
#         j.get(key, [{}]).append(dic)
#         f.close()
...
#             for item in  j.get('group_receive_list_dic_vip', [{}]):
#                 group_receive_list_vip.append(list(item.keys())[0])
...
#             # print('重新加载配置：', j)
#             # print("配置打印",admin_wx, single_block_list,group_receive_list,dict_remark_name)
#             #time.sleep(10)
#     except Exception as e:
#         print(e)
# def main():
